...
export default function DeviceManager() {
  ...
  const handlePrint = (qrSrc, deviceInfo = {}) => {
    const { id = "", brand = "", model = "", price = "", createdAt = "", details = "" } = deviceInfo;

    const priceText = price != null && price !== "" ? `${Number(price).toLocaleString("th-TH")} บาท` : "-";
    const createdAtText = createdAt
      ? new Date(createdAt).toLocaleDateString("th-TH", { year: "numeric", month: "long", day: "numeric" })
      : "-";
  ...
  // ฟังก์ชันจัดการคลิกเรียงลำดับคอลัมน์
  ...
  // กรองและเรียงลำดับข้อมูลก่อนแสดง
  const sortedDevices = React.useMemo(() => {
    let sortableDevices = [...devices];

    // filter ตาม search term (brand, model, details)
    if (searchTerm) {
...
{formVisible && (
  <div
    ...
    onClick={() => setFormVisible(false)} // คลิกที่ background ปิด modal
    aria-modal="true"
    role="dialog"
  ...
  >
    <form
      onSubmit={(e) => {
        e.stopPropagation(); // ป้องกันไม่ให้ event ลอยขึ้นไปปิด modal เมื่อคลิกในฟอร์ม
        handleSaveDevice(e);
      }}
      className="bg-white rounded-xl shadow-lg p-6 max-w-lg w-full mx-4 relative"
      onClick={(e) => e.stopPropagation()} // คลิกในฟอร์มไม่ให้ปิด modal
    >
      {/* ปุ่มปิดมุมบนขวา */}
      <button
        type="button"
      ...
      {editingDeviceId && (
        <div className="mb-5">
          <label className="block mb-2 font-semibold text-gray-700">ID อุปกรณ์:</label>
          <input
            type="text"
      ...
      <div className="mb-5">
        <label className="block mb-2 font-semibold text-gray-700">รายละเอียดเพิ่มเติม:</label>
        <textarea
          name="details"
